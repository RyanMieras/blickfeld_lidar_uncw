...
function [extrinsics extrinsicsError]= extrinsicsSolver(extrinsicsInitialGuess,extrinsicsKnownsFlag,enu,xyz)
...
if sum(extrinsicsKnownsFlag)==0  %all values are zero
    ...
    [extrinsics,R,J,CovB]   = nlinfit(enu,[xyz(:,1); xyz(:,2); xyz(:,3)],@(extrinsics,enu)enu2xyz(extrinsics,enu), extrinsicsInitialGuess,options,'ErrorModel','proportional');
...
if sum(extrinsicsKnownsFlag)>0  %if any value is not zero
    ...
    [eunknownSol,R,J,CovB]   = nlinfit(enu,[xyz(:,1); xyz(:,2); xyz(:,3)],@(extrinsicsUnknown,enu)enu2xyzforNLinFit(extrinsicsKnownsFlag,extrinsicsKnown,extrinsicsUnknown,enu),extrinsicsUnknownInitialGuess,options,'ErrorModel','proportional');
...
%% Section 3: Modified xyzToDistUV Function for nLinFit
    function [xyz]= enu2xyzforNLinFit(extrinsicsKnownsFlag,extrinsicsKnown, extrinsicsUnknown, enu)
        ...
        [xyz]= enu2xyz(intrinsics,iextrinsics,enu);
    end
